<template>
  <div class="items-center justify-center my-auto p-0 w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-lg xl:p-0 dark:bg-gray-800 dark:border-gray-700 items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
    <div class="flex flex-row gap-4 items-center justify-center mx-auto">
      ...
      <button class="inline-flex items-center justify-center w-full px-4 my-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg md:w-auto focus:outline-none hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" @click="removeTodo();$router.push('/todo/')">Remove</button>
    </div>
    <p>{{ todo.content }}</p>
  </div>
</template>
  ...
  export default {
    setup() {
      ...
      function removeTodo() {
        ...
        axios.delete('http://localhost:8081/api/v1/todo/delete/' + routes.param.id);
      }
